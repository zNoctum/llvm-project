//=====- B3TargetStreamer.cpp - B3TargetStreamer class ------------=====//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the B3TargetStreamer class.
//
//===----------------------------------------------------------------------===//
...
B3TargetStreamer::B3TargetStreamer(MCStreamer &S) : MCTargetStreamer(S) {}
B3TargetStreamer::~B3TargetStreamer() = default;

B3AsmTargetStreamer::B3AsmTargetStreamer(MCStreamer &S)
  : B3TargetStreamer(S) {}
B3AsmTargetStreamer::~B3AsmTargetStreamer() = default;

void B3TargetStreamer::outputDwarfFileDirectives() {
  for (const std::string &S : DwarfFiles)
    getStreamer().emitRawText(S);
  DwarfFiles.clear();
}

void B3TargetStreamer::closeLastSection() {
  if (HasSections)
    getStreamer().emitRawText("\t}");
}

void B3TargetStreamer::emitDwarfFileDirective(StringRef Directive) {
...
void B3TargetStreamer::changeSection(const MCSection *CurSection,
                                        MCSection *Section,
...
void B3TargetStreamer::emitRawBytes(StringRef Data) {
